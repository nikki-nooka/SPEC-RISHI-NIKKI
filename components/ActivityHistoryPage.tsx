...
const ReportModal: React.FC<{ item: ActivityLogItem, onClose: () => void }> = ({ item, onClose }) => {
    const renderReport = () => {
        switch(item.type) {
            case 'image-analysis':
                const imgAnalysisData = item.data as (AnalysisResult & { storedImageUrl?: string });
                // Use the stored image URL from Firebase if available
                return <AnalysisReport result={imgAnalysisData} imageUrl={imgAnalysisData.storedImageUrl || null} />;
            case 'prescription-analysis':
                const prescAnalysisData = item.data as (PrescriptionAnalysisResult & { storedImageUrl?: string });
                // Use the stored image URL from Firebase if available
                return <PrescriptionReport result={prescAnalysisData} imageUrl={prescAnalysisData.storedImageUrl || ""} />;
            case 'mental-health':
        ...
        <header className="w-full max-w-2xl mx-auto mb-8">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-slate-600">A log of your recent analyses (Synced to Cloud).</p>
                </div>
            </div>
        </header>
